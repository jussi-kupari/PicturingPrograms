...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname Ex2065) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
; Exercise 20.6.5
; Modify Exercise 20.6.1 or 20.6.3 so that if the user types the letter
; “q”, the animation ends.
; 
; 
; Exercise 20.6.4
...
;; stop-with-q : Posn Key-event -> Posn or Stop
(check-expect (stop-with-q (make-posn 0 0) "x") (make-posn 0 0))
(check-expect (stop-with-q (make-posn 0 0) "a") (make-posn 0 0))
(check-expect (stop-with-q (make-posn 0 0) "q") (stop-with "Game stopped by player"))

(define (stop-with-q d ke)
  (cond
    [(key=? ke "q") (stop-with "Game stopped by player")]
    [else d]))