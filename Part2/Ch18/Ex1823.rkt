...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname Ex1823) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
; 
; Exercise 18.2.2
; Modify the above animation so it also responds to some ordinary
; characters: the picture moves 5 pixels to the right in response
; to the “2” key, and 5 pixels to the left in response to the “1” key.
; 
;  -----
; 
; Exercise 18.2.3
; Modify the above animation so it stops when the user types the letter “q”.
; 
...
(check-expect (next-dot 10 "q") (stop-with "q"))

(define (next-dot d ke)
  (cond
    [(key=? ke "2") (+ d 5)]
    [(key=? ke "1") (- d 5)]
    [(key=? ke "q") (stop-with "q")]
    [else d]))
